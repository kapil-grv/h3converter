...
def h3_to_geo_boundary(h3_address, geo_json=True):
...
def h3_set_to_multi_polygon(h3_addresses, geo_json=True):
    """
...
def h3_to_geojson(h3_address):
    features = []
    GeoJSON = {}
    for i in h3_address:
        h3_hex = h3_to_geo_boundary(i)
        poly = Polygon([h3_hex])
        feature = Feature(properties={"h3_address": h3_address}, geometry=poly)
        features.append(json.loads(str(feature)))
